...
func (s *GraphLifecycleService) ensureGraph(graph GraphStore, sourcePath, spec string, syncIndex, requireReady bool) (GraphStatus, error) {
	...
	if needsIndex(status) {
		if !syncIndex {
			if requireReady {
				return GraphStatus{}, fmt.Errorf("codegraph missing or incomplete for %s; enable sync index to build it", spec)
			}
			return status, nil
		}
	...
	if stale {
		if !syncIndex {
			if requireReady {
				return GraphStatus{}, fmt.Errorf("codegraph stale for %s; enable sync index to rebuild it", spec)
			}
			status.Status = "stale"
			return status, nil
		}
