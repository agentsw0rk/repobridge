...
`)
	...
	service := codegraph.NewInspectService(codegraph.SearchServiceOptions{
		Resolver:    resolver,
		StoreOpener: openGraphStore,
	})

	status, err := service.Status("demo@v1", codegraph.GraphInspectOptions{SyncIndex: false})
	if err != nil {
		t.Fatal(err)
	}
...
`)
	...
	status, err := service.Status("demo@v1", codegraph.GraphInspectOptions{SyncIndex: true})
	if err != nil {
		t.Fatal(err)
	}
...
func TestInspectFilesFiltersAndLimitsStoredFiles(t *testing.T) {
	...
	replaceStoredGraph(t, sourceDir, codegraph.IndexResult{
		...
		Files: []codegraph.GraphFile{
			{Path: "cmd/main.go", Language: codegraph.LanguageGo, NodeCount: 1},
			{Path: "internal/cache/cache.go", Language: codegraph.LanguageGo, NodeCount: 2},
			{Path: "internal/source/source.go", Language: codegraph.LanguageGo, NodeCount: 3},
		},
	})
	resolver := &fakeSourceResolver{
		outcome: source.Outcome{Path: sourceDir, Name: "demo", Version: "v1"},
	...
	files, err := service.Files("demo@v1", codegraph.GraphInspectOptions{
		SyncIndex:  false,
		PathFilter: "internal/",
		Limit:      1,
	})
...
`)
	...
	node, err := service.Node("demo@v1", "Run", codegraph.GraphInspectOptions{SyncIndex: false, SourceLines: 2})
	if err != nil {
		t.Fatal(err)
	}
	if node.Node == nil {
...
func TestInspectNodeReportsAmbiguousNameMatches(t *testing.T) {
	...
	result, err := service.Node("demo@v1", "Run", codegraph.GraphInspectOptions{SyncIndex: false})
	if err != nil {
		t.Fatal(err)
	}
