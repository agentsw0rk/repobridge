...
func ExtractFromSource(path string, source []byte, language model.Language) (ExtractionResult, error) {
	...
	appendContainmentGraph(path, source, language, &result)
	return result, nil
}

func stableNodeID(path string, kind model.NodeKind, name string, line int) string {
